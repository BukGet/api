...
This tool is still in active development and may change at any time.
Because of this, we recommend that you backup the files that you plan on
using this tool with before running burt.
...
class BuRT(cmd.Cmd):
    ...
    def _parse_fields(self, fields, opts, data, accept_defaults=False):
        ...
        for item in fields:
            flag = False
            # Check if there is a YAML definition for this file
            ...
            # Next, step through the options that have been specified and
            # override anything that has already been set with the option that
            # was set.
            for opt, val in opts:
            ...
            # Lastly, if no flags were set, ask the user to input the
    ...
    def do_show(self, s):
        ...
        Generates the json based on what is currently in memory and will
    ...
    def do_global(self, s):
        ...
        -n (--name)     [PLUGINNAME]    Name of the plugin -- cannot contain spaces.
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        BuRT(json_file=sys.argv[1], plugin_file=sys.argv[2]).onecmd(' '.join(sys.argv[3:]))
